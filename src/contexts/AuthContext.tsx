...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (token && storedUser) {
      try {
        ...
        setIsFieldOwner(parsedUser.role === 'field_owner');
      } catch (e) {
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      setIsFieldOwner(data.user.role === 'field_owner');
  ...
  const signOut = () => {
    ...
    setUser(null);
    setIsAdmin(false);
    setIsFieldOwner(false);
  };

  return (
    <AuthContext.Provider value={{ user, isLoading, isAdmin, isFieldOwner, signUp, signIn, signOut }}>
      {children}
